<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:83:"/Applications/MAMP/htdocs/farm/public/../application/wap/view/book/orderdetail.html";i:1516084570;}*/ ?>
<!DOCTYPE html>
<html lang="zh-cmn-Hans">
	...
	<body ontouchstart>
		...
		<header class="header">
			<a class="icon-jiantou-left" href="<?php echo url('orderlist'); ?>"></a>
			<div class="header-title">已预订详情</div>
		</header>

		<div class="settlement">
			<?php if(!(empty($order['can_pay']) || (($order['can_pay'] instanceof \think\Collection || $order['can_pay'] instanceof \think\Paginator ) && $order['can_pay']->isEmpty()))): ?>
			<div class="pay-order-header">
				<p>提交支付剩余时间</p>
				<p id="time"><?php echo $order['spare_time']; ?></p>
			</div>
			<?php endif; ?>
			...
			<div class="weui-cells weui-top">
			    <div class="weui-cell">
				    ...
				    <div class="weui-cell__ft">
				    	<?php switch($order['pay_status']): case "2": ?>预付订金20%<?php break; case "1": ?>全额付款<?php break; default: ?>
						未付
				    	<!-- <label class="radio">
				    				                <input type="radio" class="groupradio" name="is_full" value="2" <?php if($order['pay_status'] == '2'): ?>checked="checked"<?php endif; ?>>
				    				                <span class="icon-checkedoff"></span> 预付订金20%
				    				            </label>
				    				            <label class="radio">
				    				                <input type="radio" class="groupradio" name="is_full" value="1" <?php if($order['pay_status'] != '2'): ?>checked="checked"<?php endif; ?>>
				...
				<div class="weui-cell">
					...
					<div class="weui-cell__ft">
						...
						<div class="originalprice">
							...
							<span class="red">
						        <small>￥</small>
						        <span><?php switch($order['pay_status']): case "1": ?><?php echo $order['order_amount']; break; case "2": ?><?php echo $order['amount20']; break; default: ?><?php echo $order['order_amount']; endswitch; ?></span> </span>
						</div>
					</div>
		...
		<div class="confirmorder-foot">
			<div class="cart-mun">
				合计：<span class="red"><small>¥</small><span><?php switch($order['pay_status']): case "1": ?><?php echo $order['order_amount']; break; case "2": ?><?php echo $order['amount20']; break; default: ?><?php echo $order['order_amount']; endswitch; ?></span></span>
			</div>
			<div class="tjdd">
				<!-- <input type="button" class="yellow-color" onclick="javascript:window.location.href='/index.php/wap/Cart/payment'" value="提交订单"> -->
				<input type="button" class="yellow-color btn-order" value="支付订单">
			</div>
		</div>
		<?php endif; ?>
		<!-- js -->
		...
		<script>
			$(function(){
				<?php if(empty($order['pay_status']) || (($order['pay_status'] instanceof \think\Collection || $order['pay_status'] instanceof \think\Paginator ) && $order['pay_status']->isEmpty())): ?>
				//提交支付剩余时间
				var all = $("#time").html();
				...
				$('.btn-order').on('click', function(){
					var order_id = "<?php echo $order['order_id']; ?>",
						is_full = $('input[name=is_full]:checked').val(),
						   data = {order_id:order_id, is_full:is_full};
				   window.location.href = "<?php echo url('payment'); ?>"+'?order_id='+order_id + '&is_full=' + is_full;
				})

				$('input[name=is_full]').on('click', function(){
					var is_full = $('input[name=is_full]:checked').val(),
						order_id = "<?php echo $order['order_id']; ?>",	
						data = { is_full:is_full, order_id:order_id};
					$.ajax({
		                url: "<?php echo url('totalPricePay'); ?>",
		                type: 'POST',
		                dataType: 'json',
		                data: data,
