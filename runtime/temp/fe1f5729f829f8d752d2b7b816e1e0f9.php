<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:78:"/Applications/MAMP/htdocs/farm/public/../application/wap/view/login/index.html";i:1516084570;}*/ ?>
<!DOCTYPE html>
<html lang="zh-cmn-Hans">
	...
	<body ontouchstart>
		...
		<div class="login-box">
			<a class="icon-jiantou-left" href="index.html"></a>
			<div class="logo">
				<a href="index.html"><img src="__PUBLIC__/skin/img/icon/logo.png"/></a>
			</div>
			<div class="login-bj">
				<div class="panel">
					...
					<div class="form-horizontal">
						<div class="form-group">
							...
							<input type="number" pattern="[0-9]*" class="form-control" id="Mobile" placeholder="手机号" required>
						</div>
						<div class="form-group">
							<i class="icon-mima"></i>
							<input type="password" class="form-control" id="password" placeholder="密　码">
							<div class="icon-wrapper">
							    <span toggle="#password" class="icon-xianshimima field-icon toggle-password"></span>
							</div>
						</div>
						<input type="button" id="login-error" class="login-btn login-btn-green mt50" value="登  录" />
						<p>
							<a class="fl green" href="<?php echo url('users/add'); ?>">注册账号</a>
							<a class="fr" href="<?php echo url('login/forgetpsw'); ?>">忘记密码</a>
						</p>
					</div>
        ...
        <!--第三方登录方式-->
        <!-- <div class="login-box-dl">
        	        <div class="login-loadmore login-loadmore-line">
        	            <span class="login-loadmore-tips">其他登陆方式</span>
        	        </div>
        	        <a class="icon-weixin" href="javascript:;"></a>
        	        <a class="icon-qq" href="javascript:;"></a>
        </div> -->
		...
		<script>
			$(function() {
				...
				//输入框判断
				$('#login-error').click(function(event) {
			    	var Mobile = $('#Mobile').val(),
					    partten = /^1[3|4|5|7|8]\d{9}$/;
					    //regExp=/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{1,18}$/;
					    regExp=/^[0-9A-Za-z]{1,18}$/;
						password = $('#password').val();
					if(Mobile == '') {
						$('#Mobile').focus();
						$.toptip('输入11位数手机号', 'warning');
						return false;
					} else if(!partten.test(Mobile)) {
						$.toptip('您输入手机号码不正确?', 'warning');
						return false;
					} else if(password.length < 3) {
						$('#password').focus();
						$.toptip('请输入6-18位字母或数字密码?', 'warning');
						return false;
						...
						$.ajax({
	                        ...
	                        success: function(data){
	                            if(data.status == 2){
	                                window.top.window.location = "<?php echo $referurl; ?>";
								}else{
