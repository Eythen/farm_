...
var refresher = {
	...
	onScrolling: function(e, pullDownEle, pullUpEle, pullUpOffset) {
		if (e.y > -(pullUpOffset)&&!pullDownEle.className.match('loadingee')) {
		...
		if (e.y > 0&&!pullUpEle.className.match('loadinge')&&!pullDownEle.className.match('loadinge')) {
			pullDownEle.classList.add("flip");
			pullDownEle.querySelector('.pullDownLabel').innerHTML = refresher.info.pullingDownLable;
			e.minScrollY = 0;
		}
		if (e.scrollerH < e.wrapperH && e.y < (e.minScrollY - pullUpOffset) &&!pullDownEle.className.match('loadinge')&&!pullUpEle.className.match('loadinge')|| e.scrollerH > e.wrapperH && e.y < (e.maxScrollY - pullUpOffset)&&!pullDownEle.className.match('loadinge')&&!pullUpEle.className.match('loadinge')) {
	...
	onRelease: function(pullDownEle, pullUpEle) {
		if (pullDownEle.className.match('loadinge')) {
			pullDownEle.classList.toggle("loadinge");
			pullDownEle.querySelector('.pullDownLabel').innerHTML = refresher.info.pullDownLable;
		}
		if (pullUpEle.className.match('loadinge')) {
			pullUpEle.classList.toggle("loadinge");
			pullUpEle.querySelector('.pullUpLabel').innerHTML = refresher.info.pullUpLable;
		}
	},
	onScrollEnd: function(pullDownEle, pullDownAction, pullUpEle, pullUpAction) {
		if (pullDownEle.className.match('flip')&&!pullDownEle.className.match('loadinge')) {
			pullDownEle.classList.add("loadinge");
			pullDownEle.classList.remove("flip");
			pullDownEle.querySelector('.pullDownLabel').innerHTML = refresher.info.loadingeLable;
			if (pullDownAction) pullDownAction();	
		}
		if (pullUpEle.className.match('flip')&&!pullUpEle.className.match('loadinge')) {
			pullUpEle.classList.add("loadinge");
			pullUpEle.classList.remove("flip");
			pullUpEle.querySelector('.pullUpLabel').innerHTML = refresher.info.loadingeLable;
