...
class Base extends Controller {
    ...
    public function _initialize() {
        ...
        if(!session('?manager_mobile')){
            if(input('manager_mobile')){
                session('manager_mobile', input('manager_mobile') );
            }
        }
        //代理商
        session('is_agent', 1);
       ...
       if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger') && $wechat_list[0]['wait_access']){
          if(session('openid') && !session('user')['manager_mobile']){
           ...
           if($wechat_config && !session('openid')){
               ...
               if($data['status'] == 1){
                   ...
                   db('cart')->where("session_id = '{$this->session_id}'")->update(array('user_id'=>$data['result']['user_id']));
                   //绑定微信与业务员手机号
                   if(!$data['result']['manager_mobile']){
    ...
    public function public_assign()
    {
       ...
       $brand_list = db('brand')->cache(true,TPSHOP_CACHE_TIME)->field('id,parent_cat_id,logo,is_hot')->where("parent_cat_id>0")->select();              
