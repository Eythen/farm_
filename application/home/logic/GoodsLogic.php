...
class GoodsLogic extends Model
{
    ...
    public function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                ...
                $sql = "SELECT * FROM  ".$prefix."goods_category ORDER BY parent_id , sort_order ASC";
    ...
    {            
        $GoodsCategory = db("goods_category"); // 实例化User对象
        $cat = $GoodsCategory->where("id = $id")->find(); // 找出他自己
        $prefix = config('database.prefix');
        // 刚新增的分类先把它的值重置一下
        if($cat['parent_id_path'] == '')
        {
            ($cat['parent_id'] == 0) && $GoodsCategory->execute("UPDATE ".$prefix."goods_category set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类               
            $GoodsCategory->execute("UPDATE ".$prefix."goods_category AS a ,".$prefix."goods_category AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");                
        ...
        $GoodsCategory->execute("UPDATE `".$prefix."goods_category` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level) WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");        
    }
    ...
    public function get_spec($goods_id)
    {
        ...
        $keys = db('spec_goods_price')->where("goods_id = $goods_id")->value("GROUP_CONCAT(`key` SEPARATOR '_') ");
        $filter_spec = array();
        if ($keys) {
            $specImage = db('spec_image')->where("goods_id = $goods_id and src != '' ")->column("spec_image_id,src");// 规格对应的 图片表， 例如颜色
            $keys = str_replace('_', ',', $keys);
            $sql = "SELECT a.name,a.order,b.* FROM yq_spec AS a INNER JOIN yq_spec_item AS b ON a.id = b.spec_id WHERE b.id IN($keys) ORDER BY b.id";
    ...
    public function getAttrInput($goods_id,$type_id)
    {
        ...
        $str = '';
        if(!isset($attributeList) || !is_array($attributeList)){
            return  $str;
        }
    ...
    { 
        $arr = db('spec_item')->where("spec_id = $spec_id")->order('id')->select(); 
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
        ...
        //dump($spec_arr);       
        // 排序
        if(!isset($spec_arr) || !is_array($spec_arr)){
            return  false;
        }
       ...
       /*  dump($spec_arr2);        
         ...
         $keySpecGoodsPrice = db('SpecGoodsPrice')->where('goods_id = '.$goods_id)->column('key,key_name,price,store_count,bar_code,sku');//规格项
//dump($keySpecGoodsPrice);
       ...
       foreach ($spec_arr2 as $k => $v) 
       {
           ...
           // dump($item_key) ;        
            ...
            if(!isset($keySpecGoodsPrice[$item_key]['price'])){
                $keySpecGoodsPrice[$item_key]['price'] = 0; // 价格默认为0
            }
            if(!isset($keySpecGoodsPrice[$item_key]['store_count'])){
                $keySpecGoodsPrice[$item_key]['store_count'] = 0; //库存默认为0
                $keySpecGoodsPrice[$item_key]['sku'] = '';
            }
    ...
    function GetSpecCheckboxList($type_id, $checked = array()){
        $list = db('Spec')->where("type_id = $type_id")->order('`order` desc')->select();        
        //$list = db('Spec')->where("1=1")->order('`order` desc')->select();        
        $str = '';
    ...
    function GetAttrCheckboxList($type_id, $checked = array()){                
        $list = db('goods_attribute')->where("type_id = $type_id and attr_index > 0 ")->order('`order` desc')->select();        
        //$list = db('Spec')->where("1=1")->order('`order` desc')->select();        
        $str = '';
...
//        $cat_list =  db('goods_category')->field('id,parent_id,level')->select();
...
//             $cat_level_arr[$cat_list[$grandfather_id]['level']] = $grandfather_id;
        ...
        $cat_list = db('goods_category')->field('id')->select();
        $cat_info = db('goods_category')->field('id,parent_id_path')->find($cat_id);
        $cat_level_arr = explode('_',$cat_info['parent_id_path']);
    ...
    function getSortBrands()
    {
        ...
        $brandIdArr =  db("Brand")->where("name in (select `name` from `".config('database.prefix')."brand` group by name having COUNT(id) > 1)")->column('id,cat_id'); 
        $goodsCategoryArr = db('goodsCategory')->where("level = 1")->column('id,name');
        $nameList = array();
    ...
    {
        $categoryList =  db("goods_category")->field('id,name,parent_id,level')->select();
        $nameList = array();
