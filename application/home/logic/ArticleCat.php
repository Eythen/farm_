    ...
    public function article_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
        ...
        if ($res === NULL)
        {
            ...
            if ($data === false)
            {
                ...
                $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children FROM ".$tb_pre."article_cat AS c".
                    " LEFT JOIN ".$tb_pre."article_cat AS s ON s.parent_id=c.cat_id $where ".
                    " GROUP BY c.cat_id  ORDER BY parent_id, sort_order";
                $res = db('article_cat')->query($sql);
        ...
        else
        {
            foreach ($options AS $key => $value)
            {
                if ($value['level'] > 0)
                {
                    $options[$key]['cat_name'] = str_repeat('&nbsp;', $value['level'] * 4) . $value['cat_name'];
                }
