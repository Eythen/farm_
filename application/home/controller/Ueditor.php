...
*/
        ...
        $path = htmlspecialchars(input('dir', 1), ENT_QUOTES); 
        ...
        $file_dir_path = BASE_PATH . 'public' . DS . $path . DS .  $this->savePath;
       ...
       // 获取表单上传文件 例如上传了001.jpg
        ...
        $return_data['url'] = '/public' . "/" . $path . "/" .  $this->savePath . date('Ymd') .'/'.  $info->getFilename();
    ...
    public function imageUp()
    {
        ...
        // 上传图片框中的描述表单名称，
        $_POST['pictitle'] = input('pictitle', '');
        $_POST['dir'] = input('dir', '');
        $_POST['Filename'] = input('name');
        ...
        $filename = htmlspecialchars($_POST['Filename'], ENT_QUOTES); 
        if(!empty($_POST['dir'])){
            $path = 'upload/'.$path.$_POST['dir'].'/'.date('Y');
        } 
        else{
            $path = 'upload/'.$path.date('Y');
        } 

        $file_dir_path = BASE_PATH . 'public' . '/' . $path . '/';
       
       // 获取表单上传文件 例如上传了001.jpg
        //logWrite($pictitle);
        //logWrite($filename);
        //logWrite($file_dir_path);
        ...
        //$file = request()->file('uploadify');
        //logWrite($file);die;


        // 移动到框架应用根目录/public/uploads/ 目录下
        $info = $file->validate([ 'size' => 20000000, 'ext' => 'gif,png,jpg,jpeg,bmp' ])->move($file_dir_path);
        //$info = $file->move($file_dir_path);

        if ($info) {
            $state = "SUCCESS";         
        } else {
            //$state = "ERROR" . $info->getError();
            $state = "SUCCESS";
        }
    	...
    	if($this->savePath=='Goods/'){
    		...
    		$imgresource = $file_dir_path . $info->getExtension();
