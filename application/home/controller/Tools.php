        ...
        foreach ($tables as $table) {
            ...
            for ($i = 0; $i <= $page; $i++) {
                ...
                foreach ($query as $val) {
                    ...
                    if (strlen($pre) + strlen($sqlNo) + strlen($sqlTable) + strlen($outstr) + strlen($temSql) > config("CFG_SQL_FILESIZE")) {
...
//			exit($file);
    ...
    public function restoreData() {
    		...
    		while (!feof($file)) {
	    	   ...
	    	   $execute++;
	    	   		if ($execute > 500) {
			    		...
			    		//echo json_encode(array("status" => 1, "info" => '如果导入SQL文件卷较大(多)导入时间可能需要几分钟甚至更久，请耐心等待导入完成，导入期间请勿刷新本页，当前导入进度：<font color="red">已经导入' . $imported . '条Sql</font>', "url" => url('Database/restoreData', array(get_randomstr(5) => get_randomstr(5)))));
    	...
    	unset($_SESSION['cacheRestore']);
    	$this->success("导入成功，耗时：{$time} 秒钟", url('Tools/restore'));
    }
    ...
    public function restoreUpload()
    {
    	...
    	if (!$info) { // 上传错误提示错误信息
    		...
    		if (file_exists($file_path_full)) {
    			$this->success("上传成功", url('Tools/restore'));
    		} else {
    			$this->error('文件不存在');   
    		}
    	}
    ...
    public function delSqlFiles() {
    	...
    	if($a){
    		$this->success("已删除：" . implode(",", $files), url('Tools/restore'));
    	}else{
    		$this->error("删除失败：" . implode(",", $files), url('Tools/restore'));
    	}	
    }
    ...
    public function regionHandle(){
    	$data = input('post.');
    	$id = input('id');
    	$referurl =  isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : url("Tools/region");
    	if(empty($id)){
    		...
    		db('region')->where("id=$id or parent_id=$id")->delete();
    		$this->success("操作成功", $referurl);
    	}
    }
}