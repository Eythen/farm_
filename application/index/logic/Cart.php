...
class Cart extends Model
{
    ...
    {       
        ...
        $goods = db('Goods')->where("goods_id = $goods_id")->find(); // 找出这个商品        
        $specGoodsPriceList = db('SpecGoodsPrice')->where("goods_id = $goods_id")->column("key,key_name,price,store_count,sku"); // 获取商品对应的规格价钱 库存 条码

		$where = " session_id = '$session_id' ";
        $user_id = $user_id ? $user_id : 0;
		if($user_id)
		 $where .= "  or user_id= $user_id ";
        $catr_count = db('Cart')->where($where)->count(); // 查找购物车商品总数量
        if($catr_count >= 20) 
        ...
        {
            $flash_sale = db('flash_sale')->where("id = {$goods['prom_id']} and ".time()." > start_time and ".time()." < end_time and goods_num > buy_num")->find(); // 限时抢购活动
            if($flash_sale){
                $cart_goods_num = db('Cart')->where("($where) and goods_id = {$goods['goods_id']}")->value('goods_num');
                // 如果购买数量 大于每人限购数量
        ...
        $catr_goods = db('Cart')->where($where)->find(); // 查找购物车是否已经存在该商品
       ...
       {          
           // 如果购物车的已有数量加上 这次要购买的数量  大于  库存输  则不再增加数量
            if(($catr_goods['goods_num'] + $goods_num) > $goods['store_count'])
                $goods_num = 0;
            $result = db('Cart')->where("id =".$catr_goods[id])->update(  array("goods_num"=> ($catr_goods['goods_num'] + $goods_num)) ); // 数量相加        
    ...
    {                   
        ...
        $cartList = db('Cart')->where($where)->select();  // 获取购物车商品 
...
function cart_freight2($shipping_code,$province,$city,$district,$weight)
{
   ...
   // 先根据 镇 县 区找 shipping_area_id   
      $shipping_area_id = db('AreaRegion')->where("shipping_area_id in (select shipping_area_id from  ".C('DB_PREFIX')."shipping_area where shipping_code = '$shipping_code') and region_id = {$district}")->value('shipping_area_id');
    
    // 先根据市区找 shipping_area_id
   if($shipping_area_id == false)    
      $shipping_area_id = db('AreaRegion')->where("shipping_area_id in (select shipping_area_id from  ".C('DB_PREFIX')."shipping_area where shipping_code = '$shipping_code') and region_id = {$city}")->value('shipping_area_id');

   // 市区找不到 根据省份找shipping_area_id
   if($shipping_area_id == false)
        $shipping_area_id = db('AreaRegion')->where("shipping_area_id in (select shipping_area_id from  ".C('DB_PREFIX')."shipping_area where shipping_code = '$shipping_code') and region_id = {$province}")->value('shipping_area_id');
   ...
   {           
       ...
       $shipping_area_id = db("ShippingArea")->where("shipping_code = '$shipping_code' and is_default = 1")->value('shipping_area_id');
   }
   if($shipping_area_id == false)
       return 0;
   /// 找到了 shipping_area_id  找config       
   $shipping_config = db('ShippingArea')->where("shipping_area_id = $shipping_area_id")->value('config');
   ...
   return $freight;  
    ...
    {
        $couponlist = db('CouponList')->where("uid = $user_id and id = $coupon_id")->find(); // 获取用户的优惠券
        if(empty($couponlist)) {
            if($mode == 1) return 0;    
            return array('status'=>1,'msg'=>'','result'=>0);
        }            
        
        $coupon = db('Coupon')->where("id = {$couponlist['cid']}")->find(); // 获取 优惠券类型表
    ...
    {
        $couponlist = db('CouponList')->where("code = '$couponCode'")->find(); // 获取用户的优惠券
        if(empty($couponlist)) 
            return array('status'=>-9,'msg'=>'优惠券码不存在','result'=>'');
        $coupon = db('Coupon')->where("id = {$couponlist['cid']}")->find(); // 获取优惠券类型表
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price)
    {
        ...
        $order_count = db('Order')->where("user_id= $user_id and order_sn like '".date('Ymd')."%'")->count(); // 查找购物车商品总数量
        if($order_count >= 50) 
            return array('status'=>-9,'msg'=>'一天只能下50个订单','result'=>'');            
        
         // 0插入订单 order
        $address = db('UserAddress')->where("address_id = $address_id")->find();
        $shipping = db('Plugin')->where("code = '$shipping_code'")->find();
        $data = array(
        ...
        // 1插入order_goods 表
        $cartList = db('Cart')->where("user_id = $user_id and selected = 1")->select();
        foreach($cartList as $key => $val)
        {
           $goods = db('goods')->where("goods_id = {$val['goods_id']} ")->find();
           ...
           $order_goods_id              = db("OrderGoods")->insertGetId($data2); 
           // 扣除商品库存  扣除库存移到 付完款后扣除
           //db('Goods')->where("goods_id = ".$val['goods_id'])->setDec('store_count',$val['goods_num']); // 商品减少库存
        } 
        ...
        if($coupon_id > 0){
        	...
        	$data3['use_time'] = time();
        	db('CouponList')->where("id = $coupon_id")->update($data3);
                $cid = db('CouponList')->where("id = $coupon_id")->getField('cid');
                db('Coupon')->where("id = $cid")->setInc('use_num'); // 优惠券的使用数量加一
        }
        // 3 扣除积分 扣除余额
        if($car_price['pointsFee']>0)
        	db('Users')->where("user_id = $user_id")->setDec('pay_points',($car_price['pointsFee'] * tpCache('shopping.point_rate'))); // 消费积分 
        if($car_price['balance']>0)
        	db('Users')->where("user_id = $user_id")->setDec('user_money',$car_price['balance']); // 抵扣余额         
        // 4 删除已提交订单商品
        db('Cart')->where("user_id = $user_id and selected = 1")->delete();
        ...
        // 如果使用了积分或者余额才记录
        ($data4['user_money'] || $data4['pay_points']) && db("AccountLog")->insert($data4);           
    ...
    public function cart_count($user_id,$mode = 0){
        $count = db('Cart')->where("user_id = $user_id and selected = 1")->count();
   ...
   {
       $group_buy = db('GroupBuy')->where("goods_id = $goods_id and ".time()." >= start_time and ".time()." <= end_time ")->find(); // 找出这个商品                      
       if(empty($group_buy))       
            return 0;
   ...
   public function login_cart_handle($session_id,$user_id)
   {
	   if(empty($session_id) || empty($user_id))
        ...
        db('cart')->where("session_id = '$session_id'")->update(array('user_id'=>$user_id));                    
        
        $Model = db();
        // 查找购物车两件完全相同的商品
        $cart_id_arr = $Model->query("select id from `".config('database.prefix')."cart` where user_id = $user_id group by  goods_id,spec_key having count(goods_id) > 1");
        if(!empty($cart_id_arr))
        {
            $cart_id_arr = get_arr_column($cart_id_arr, 'id');
            $cart_id_str = implode(',', $cart_id_arr);
            db('cart')->delete($cart_id_str); // 删除购物车完全相同的商品
        }
   }
}